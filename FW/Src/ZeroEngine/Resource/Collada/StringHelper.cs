    ...
    class CStringHelper
    {
			...
			string functionName, int startPos)
		{
			...
			for (int pos = startPos; pos < functionName.Length; pos++)
			{
				...
				if (CStringHelper.IsUppercaseLetter(functionName[pos]) == false)
					break;
		...
		public static string SplitFunctionNameToWordString(string functionName)
		{
			...
			for (int pos = 0; pos < functionName.Length; pos++)
			{
				...
				if (pos == 0 ||
					pos == 1 && CStringHelper.IsUppercaseLetter(functionName[1]) &&
					CStringHelper.IsUppercaseLetter(functionName[0]) ||
					pos == 2 && CStringHelper.IsUppercaseLetter(functionName[2]) &&
					CStringHelper.IsUppercaseLetter(functionName[1]) &&
					CStringHelper.IsUppercaseLetter(functionName[0]))
					ret += CStringHelper.ToUpper(letter);
					// Found uppercase letter?
				else if (CStringHelper.IsUppercaseLetter(letter) &&
					...
					CStringHelper.IsSpaceOrPunctuation(letter) == false &&
					ret.EndsWith(" ") == false)
				{
					...
					if (abbreviationLength > 1)
					{
						...
						ret += " " + CStringHelper.ToLower(letter);
				} // else if
				else
					// Just add letter
					ret += letter;
